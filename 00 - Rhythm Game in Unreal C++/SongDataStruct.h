...
public:
    ...
    /** LevelSequence asset containing note data */
    UPROPERTY(EditAnywhere)
    ...
    /** How early (in beats) a note can be hit without it being missed */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (ClampMax = "0.0"))
    float EarliestRegisteredHitDeltaInBeats = -0.2f;

    /** How late (in beats) a note can be hit without it being missed */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (ClampMin = "0.0"))
